...
"""
Description:    Train the model using the VAV2 feature.
Usage:          python3 run_VAV2.py --load-VAV2 ./VAV2.pkl --load-classes ./y.pkl
"""
...
def check_args(args):
    if args.load_VAV2 is None and args.load_classes is None:
        exit('Please run the program using --load-VAV2, and --load-classes')
...
def train_WhiVo_Model(VAV2_2d, INPUT_SHAPE, NUM_CLASSES, VAV2_weights, y):
    
    if not isdir("./VAV2_weights"):
        makedirs("./VAV2_weights")
    if not isdir("./pics"):
        makedirs("./pics")

    early_stopping = EarlyStopping(monitor="val_accuracy", patience=100, restore_best_weights=True)

    checkpoint1 = ModelCheckpoint('./VAV2_weights/VAV2-model-{epoch:03d}-{accuracy:03f}-{val_accuracy:03f}.h5',
    ...
    batch_size = 256
    if VAV2_weights is None:
        df_size_history = model.fit(VAV2_2d[:te_cut], to_categorical(y[:te_cut]),
        ...
        model.load_weights(VAV2_weights)
    ...
    plot_model(model, to_file='./pics/VAV2_model.png', show_shapes='True')
...
if __name__ == '__main__':
    ...
    if args.load_VAV2 is None:
        exit('Please provide values for arguments (--load-VAV2) when loading in your own data')
    else:
    ...
    train_WhiVo_Model(VAV2_2d, INPUT_SHAPE, NUM_CLASSES, VAV2_weights, y)
