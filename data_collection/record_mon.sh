...
command_subdir=$command_dir/*
wake_word_file=$wake_word_dir/*

# For each voice
for voice in {1..100}; do
  ...
  for result_dir in ${result_dir[@]}; do
    ...
    for variant_file in ${command_dir}; do
      ...
      while ! timeout --foreground 60s sox -t alsa plughw:2 -d -c 1 -r 44100 $result_dir/sarah_$(printf '%03d' ${voice}).wav silence 1 0.1 5% 1 3.0 5%; do  
        echo -e "while"
